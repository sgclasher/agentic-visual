...
// It needs to receive searchParams to know which use case to potentially pre-select
export default async function UseCaseDetailViewPage({ searchParams }: { searchParams?: { [key: string]: string | string[] | undefined } }) {
  const initialWorkflowData = await getWorkflowData();
  const selectedUseCaseId = searchParams?.usecase as string | undefined;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6 text-center">Use Case Detail View</h1>
      {/* Pass initial data and the selected ID from params to the client component */}
      <UseCaseDetailView 
        initialWorkflowData={initialWorkflowData} 
        initialSelectedUseCaseId={selectedUseCaseId}
      />
    </div>
  );
} 